...
void movimento2(char stanza[][size])
{
    ...
    */
    ...
    switch(ind_max)
    {
        case 0: c.x++;if(stanza[c.x][c.y]=='X')c.x--;break; //Se il robot non e' stato precedentemente a SUD allora... - Caso in cui il robot scelga di andare a NORD.

        case 1: c.x--;if(stanza[c.x][c.y]=='X')c.x++;break; //Se il robot non e' stato precedentemente a NORD allora... - Caso in cui il robot scelga di andare a SUD.

        case 2: c.y++;if(stanza[c.x][c.y]=='X')c.y--;break; //Se il robot non e' stato precedentemente a OVEST allora... - Caso in cui il robot scelga di andare a EST.

        case 3: c.y--;if(stanza[c.x][c.y]=='X')c.y++;break; //Se il robot non e' stato precedentemente a EST allora... - Caso in cui il robot scelga di andare a OVEST.
    }
    stanza[c.x][c.y]='R';
}
