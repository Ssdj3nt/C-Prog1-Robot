...
void movimento2(char stanza[size][size])
{
    ...
    */
    ...
    switch(ind_max)
    {
        case 0:if(p!=2)c.x++;p=1;if(stanza[c.x][c.y]=='X')c.x--;p=0;break; //NORD

        case 1:if(p!=1)c.x--;p=2;if(stanza[c.x][c.y]=='X')c.x++;p=0;break; //SUD

        case 2:if(p!=4)c.y++;p=3;if(stanza[c.x][c.y]=='X')c.y--;p=0;break; //EST

        case 3:if(p!=3)c.y--;p=4;if(stanza[c.x][c.y]=='X')c.y++;p=0;break; //OVEST
    }stanza[c.x][c.y]='R';
